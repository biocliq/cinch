...
import com.zitlab.palmyra.cinch.exception.GenericValidation;
...
public class ConfigHelper  {
	...
	private void verifyKey(String keyName, TTUniqueKey value, TupleType citype, Schema schema) {
		...
		int count = 0;
		Map<String, UniqueKey> uqKeys = citype.getUniqueKeyMap();
		
		for( Entry<String, UniqueKey> keys: uqKeys.entrySet()) {
			UniqueKey key = keys.getValue();
			count = 0;
			...
			if(count == size) {
				...
				UniqueKey val = uqKeys.remove(_key);
